...
const loadLocalStorageData = () => {
  try {
    ...
    console.log("Error loading local storage data:", error); // 데이터 로드 중 에러
  }
};
...
const generateApiResponse = async (incomingMessageDiv) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.error.message); // API 응답 실패
    ...
    if (error.message.includes("NetworkError")) {
      textElement.innerText = "Network error. Please check your connection."; // 네트워크 에러
    } else {
      textElement.innerText = "Error occurred. Please try again later."; // 일반 에러
    }
